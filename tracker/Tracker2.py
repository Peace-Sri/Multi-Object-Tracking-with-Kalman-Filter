...
class Tracker(object):
	...
	def __init__(self, dist_threshold=150, max_frame_skipped=30 ):
		...
		if (detections.shape[0] >= self.previous_index_tracks) and (detections.shape[0] != 0): #Check if index of tracks are greater or equal than previous one
			...
			for i in range(len(assignment)): #Loop for finding un_assignments
				if assignment[i] != -1: # Check If assignments has been assigned
					print(f"X: {detections[assignment[i]][0]}")
					if (detections[assignment[i]][0] > MaxRoi):
						self.tracks[assignment[i]].red_zone += 1

			for i in range(len(assignment)): #Loop for finding un_assignments
				if assignment[i] != -1: # Check If assignments has been assigned
					if (cost[i][assignment[i]] > self.dist_threshold) and (self.tracks[assignment[i]].step > 20): # If assigned track is exceeding to distance threshold, It will be declared to unassigned track
						print(f"COST: {cost[i][assignment[i]]}")
